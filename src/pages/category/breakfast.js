...
function breakfast() {
  ...
  const feedBreakfast = async () => {
    const checkLocal = localStorage.getItem('breakfast40');

    if (checkLocal) {
      setBreakfastData(JSON.parse(checkLocal));
    } else {
      ...
      const { data } = api;
      localStorage.setItem('breakfast40', JSON.stringify(data.recipes));
      setBreakfastData(data.recipes);
    }
  };
  useEffect(() => {
    feedBreakfast();
  ...
  return (
    <div className="bg-gray-100">
      ...
      <div className="flex items-center justify-center text-justify gap-2 sm:gap-4 px-5">
        <Button
          variant="text"
          color="blue-gray"
