...
function paleogenicPage({}) {
  const [paleoPage, setPaleoPage] = useState([]);
  ...
  const feedPaleo = async () => {
    const checkLocal = localStorage.getItem('Paleo40');

    if (checkLocal) {
      setPaleoPage(JSON.parse(checkLocal));
    } else {
      ...
      const { data } = api;
      localStorage.setItem('Paleo40', JSON.stringify(data.recipes));
      setPaleoPage(data.recipes);
    }
  };
  return (
