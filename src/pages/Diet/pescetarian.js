...
function pescetarianPage() {
  const [pescPage, setPescPage] = useState([]);
  ...
  const feedPescetarian = async () => {
    const checkLocal = localStorage.getItem('Pescetarian40');

    if (checkLocal) {
      setPescPage(JSON.parse(checkLocal));
    } else {
      ...
      const { data } = api;
      localStorage.setItem('Pescetarian40', JSON.stringify(data.recipes));
      setPescPage(data.recipes);
    }
  };
  useEffect(() => {
    feedPescetarian();
...
export default pescetarianPage;
