...
function glutenFreePage() {
  const [gfPage, setGfPage] = useState([]);
  ...
  const feedGF = async () => {
    const checkLocal = localStorage.getItem('glutenFree40');

    if (checkLocal) {
      setGfPage(JSON.parse(checkLocal));
    } else {
      ...
      const { data } = api;
      localStorage.setItem('glutenFree40', JSON.stringify(data.recipes));
      setGfPage(data.recipes);
    }
  };
  useEffect(() => {
    feedGF();
