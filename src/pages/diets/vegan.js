...
function veganPage() {
  const [vegPage, setVegPage] = useState([]);
  ...
  const feedVegan = async () => {
    const checkLocal = localStorage.getItem('Vegan40');

    if (checkLocal) {
      setVegPage(JSON.parse(checkLocal));
    } else {
      ...
      const { data } = api;
      localStorage.setItem('Vegan40', JSON.stringify(data.recipes));
      setVegPage(data.recipes);
    }
  };
  useEffect(() => {
    feedVegan();
